...
module.exports.setup = function(settings) {
    ...
    */
    ...
    if (settings.plugins && settings.plugins.external && settings.plugins.external.enable) {
...
module.exports.checkAllServices = function() {
    ...
    */
    setTimeout(function() {
        var statusTab = _.map(status.services, function(value, key) {
            return value;
        });

        status.summarize = {};
        status.summarize.lastupdate = status.lastupdate;
        status.summarize.up = _.reduce(_.select(status.services, function(data) {
            return data.status == 'up';
        }), function(memo, num) {
            return memo + 1;
        }, 0);
        status.summarize.critical = _.reduce(_.select(status.services, function(data) {
            return data.status == 'critical';
        }), function(memo, num) {
            return memo + 1;
        }, 0);
        status.summarize.maintenance = _.reduce(_.select(status.services, function(data) {
            return data.status == 'maintenance';
        }), function(memo, num) {
            return memo + 1;
        }, 0);
        status.summarize.down = _.reduce(_.select(status.services, function(data) {
            return data.status == 'down';
        }), function(memo, num) {
            return memo + 1;
        }, 0);
        status.summarize.unknown = _.reduce(_.select(status.services, function(data) {
...
module.exports.configClient = function(req, res) {
    res.status(200).send(JSON.stringify(settings.client));
...
module.exports.pluginsClient = function(req, res) {
    ...
    res.status(200).send(JSON.stringify(plugins));
...
var date_formats = {
    past: [{
        ...
        {
            ceiling: 86400,
            text: "$hours hours, $minutes minutes and $seconds seconds ago"
        },
        {
            ceiling: 2629744,
            text: "$days days, $hours hours, $minutes minutes and $seconds seconds ago"
        },
        {
            ceiling: 31556926,
            text: "$months months, $days days, $hours hours, $minutes minutes and $seconds seconds ago"
        },
        {
            ceiling: null,
            text: "$years years ago, $months months, $days days, $hours hours, $minutes minutes and $seconds seconds"
        }
    ],
    future: [{
            ceiling: 60,
